...
export class AddEditDepartmentComponent implements OnInit {
  ...
  constructor(private dialogRef: MatDialogRef<AddEditDepartmentComponent>,
    @Inject (MAT_DIALOG_DATA) public data:Department,
   
    private workspaceService: WorkspaceService, private formBuilder: FormBuilder, public accountService: AccountService,private route: ActivatedRoute){}
  ...
  createDepartment(): void {
    ...
    if(this.data){
      ...
      this.workspaceService.updateDepartment(this.departmentForm.value,this.data.departmentId)
      .subscribe((response: APIResponse) => {
        if (response.responseCode === 200) {
          console.log('Department Updated successfully. ID:', response.result);
